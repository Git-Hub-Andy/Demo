...
- (void)setupBottomLine{
    ...
    UILabel *label = self.titleLabels[self.style.selectIndex];
    //切换文字颜色
    label.textColor = self.style.selectedColor;
    
    //保存最新Label的下标值
    self.currentIndex = label.tag - 10;
    
    self.bottomLine.frame = CGRectMake(label.frame.origin.x + ((label.frame.size.width - self.style.bottomLineW) / 2.0),self.bounds.size.height - self.style.bottomLineH, self.style.bottomLineW, self.style.bottomLineH);
}
