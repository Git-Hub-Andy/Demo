...
//        pickerLabel.font = [UIFont systemFontOfSize:10.];
    ...
    ((UIView *)[self.myPickerView.subviews objectAtIndex:1]).backgroundColor = [UIColor lightGrayColor];
    ((UIView *)[self.myPickerView.subviews objectAtIndex:2]).backgroundColor = [UIColor lightGrayColor];
    return pickerLabel;
}
...
- (IBAction)clockDetermine:(id)sender {
    ...
    WS(weakSelf);
    if (self.block) {
        weakSelf.block(weakSelf,str,[weakSelf.myPickerView selectedRowInComponent:0]);
    }
    [self close];
}
#pragma mark 取消
