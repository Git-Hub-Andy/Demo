...
+ (NSURLSessionTask *)GET:(NSString *)URL parameters:(id)parameters success:(Success)success failure:(Failed)failure {
    return [self GET:URL parameters:parameters responseCache:nil success:success failure:failure];
}

#pragma mark - POST请求无缓存
+ (NSURLSessionTask *)POST:(NSString *)URL parameters:(id)parameters success:(Success)success failure:(Failed)failure {
    return [self POST:URL parameters:parameters responseCache:nil success:success failure:failure];
}

#pragma mark - GET请求自动缓存
+ (NSURLSessionTask *)GET:(NSString *)URL parameters:(id)parameters responseCache:(Cache)responseCache success:(Success)success failure:(Failed)failure {
    //读取缓存
...
+ (NSURLSessionTask *)POST:(NSString *)URL parameters:(id)parameters responseCache:(Cache)responseCache success:(Success)success failure:(Failed)failure {
    //读取缓存
    responseCache!=nil ? responseCache([NetworkCache httpCacheForURL:URL parameters:parameters]) : nil;
...
#pragma mark - 上传文件
+ (NSURLSessionTask *)uploadFileWithURL:(NSString *)URL parameters:(id)parameters name:(NSString *)name filePath:(NSString *)filePath progress:(Progress)progress success:(Success)success failure:(Failed)failure {    
...
+ (NSURLSessionTask *)uploadImagesWithURL:(NSString *)URL parameters:(id)parameters images:(NSArray<UIImage *> *)images imageScale:(CGFloat)imageScale progress:(Progress)progress success:(Success)success failure:(Failed)failure {
    NSURLSessionTask *sessionTask = [_sessionManager POST:URL parameters:parameters constructingBodyWithBlock:^(id<AFMultipartFormData>  _Nonnull formData) {
        for (int i = 0; i < images.count; i++) {
            ...
            NSString *name = [NSString stringWithFormat:@"photos[%d]",i];
...
#pragma mark - 下载文件
+ (NSURLSessionTask *)downloadWithURL:(NSString *)URL fileDir:(NSString *)fileDir progress:(Progress)progress success:(void(^)(NSString *))success failure:(Failed)failure {
